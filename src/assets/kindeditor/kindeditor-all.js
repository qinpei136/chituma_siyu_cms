...
(function (window, undefined) {
  ...
  function _inString(val, str, delimiter) {
    ...
    return (
      (delimiter + str + delimiter).indexOf(delimiter + val + delimiter) >= 0
    );
  }
  ...
  K.options = {
    ...
    layout:
      '<div class="container"><div class="toolbar"></div><div class="edit"></div><div class="statusbar"></div></div>',
  };
  ...
  _extend(KEvent, {
    init: function (el, event) {
      ...
      if (!self.relatedTarget && self.fromElement) {
        self.relatedTarget =
          self.fromElement === self.target ? self.toElement : self.fromElement;
      }
      ...
      if (
        !self.which &&
        (self.charCode || self.charCode === 0 ? self.charCode : self.keyCode)
      ) {
      ...
      if (!self.which && self.button !== undefined) {
        self.which =
          self.button & 1 ? 1 : self.button & 2 ? 3 : self.button & 4 ? 2 : 0;
      }
      switch (self.which) {
        case 186:
          self.which = 59;
  ...
  function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
    ...
    if (htmlTags) {
      ...
      if (!htmlTagMap.script) {
        html = html.replace(
          /(<(?:script|script\s[^>]*)>)([\s\S]*?)(<\/script>)/gi,
          ""
        );
      }
      if (!htmlTagMap.style) {
        html = html.replace(
          /(<(?:style|style\s[^>]*)>)([\s\S]*?)(<\/style>)/gi,
          ""
        );
      }
    }
    ...
    html = html.replace(re, function ($0, $1, $2, $3, $4, $5, $6) {
      ...
      if (_BLOCK_TAG_MAP[tagName] && !_PRE_TAG_MAP[tagName]) {
        if (wellFormatted) {
          ...
          endNewline = "\n";
          for (
            var i = 0, len = startSlash ? tagStack.length : tagStack.length - 1;
            i < len;
            i++
          ) {
  ...
  function _clearMsWord(html, htmlTags) {
    html = html
      ...
      .replace(/<(?:table|td)[^>]*>/gi, function (full) {
  ...
  function _computedCss(el, key) {
    ...
    if (win.getComputedStyle) {
      ...
      val =
        style[camelKey] || style.getPropertyValue(key) || el.style[camelKey];
      ...
      this.each(function () {
        if (_hasClass(this, cls)) {
          this.className = _trim(
            this.className.replace(new RegExp("(^|\\s)" + cls + "(\\s|$)"), " ")
          );
        }
      });
      return this;
  ...
  _extend(KRange, {
    ...
    get: function (hasControlRange) {
      ...
      rng.setEndPoint(
        "StartToStart",
        _getEndRange(range.startContainer, range.startOffset)
      );
      rng.setEndPoint(
        "EndToStart",
    ...
    shrink: function () {
      ...
      while (
        self.startContainer.nodeType == 1 &&
  ...
  function _hasAttrOrCss(knode, map) {
    return (
      _hasAttrOrCssByKey(knode, map, "*") || _hasAttrOrCssByKey(knode, map)
    );
  }
  ...
  function _inPreElement(knode) {
    while (knode && knode.name != "body") {
      if (
        _PRE_TAG_MAP[knode.name] ||
        (knode.name == "div" && knode.hasClass("ke-script"))
      ) {
        return true;
      }
      knode = knode.parent();
  ...
  _extend(KCmd, {
    ...
    select: function (hasDummy) {
      ...
      if (hasDummy && sc.nodeType == 1 && range.collapsed) {
        ...
        if (_WEBKIT) {
          ...
          if (
            K(sc).isInline() ||
            (so > 0 && K(children[so - 1]).isInline()) ||
            (children[so] && K(children[so]).isInline())
          ) {
    ...
    createlink: function (url, type) {
      ...
      var html =
        '<a href="' + _escape(url) + '" data-ke-src="' + _escape(url) + '" ';
      if (type) {
    ...
    function (i, name) {
      KCmd.prototype[name] = function (val) {
        ...
        if (
          ...
          _inArray(
            name,
            "justifyleft,justifycenter,justifyright,justifyfull".split(",")
          ) >= 0
        ) {
          self.selection();
        }
      ...
      if (options.autoScroll) {
        if ((_IE && _V < 7) || _QUIRKS) {
          ...
          K(self.win).bind("scroll", function (e) {
            ...
            self.pos(
              _removeUnit(self.x) + diffX,
              _removeUnit(self.y) + diffY,
              false
            );
          });
        } else {
  ...
  _extend(KEdit, KWidget, {
    init: function (options) {
      ...
      var themesPath = _undef(options.themesPath, ""),
        ...
        isDocumentDomain =
          location.protocol != "res:" &&
          location.host.replace(/:\d+/, "") !== document.domain,
        srcScript =
          "document.open();" +
          (isDocumentDomain
      ...
      self.tabIndex = isNaN(parseInt(options.tabIndex, 10))
        ? self.srcElement.attr("tabindex")
    ...
    html: function (val, isFull) {
      ...
      if (self.designMode) {
        ...
        if (val === undefined) {
          if (isFull) {
            val =
              "<!doctype html><html>" + body.parentNode.innerHTML + "</html>";
          } else {
            val = body.innerHTML;
          }
  ...
  _extend(KMenu, KWidget, {
    ...
    addItem: function (item) {
      ...
      if (self.centerLineMode) {
        centerDiv = K(
          '<div class="ke-inline-block ke-menu-item-center"></div>'
        );
        if (height) {
      ...
      if (iconClass !== "") {
        leftDiv.html(
          '<span class="ke-inline-block ke-toolbar-icon ke-toolbar-icon-url ' +
            iconClass +
            '"></span>'
        );
      }
  ...
  _extend(KUploadButton, {
    init: function (options) {
      var self = this,
        ...
        target =
          options.target || "kindeditor_upload_iframe_" + new Date().getTime();
      options.afterError =
        options.afterError ||
        function (str) {
          alert(str);
      ...
      iframe.bind("load", function () {
        ...
        try {
          ...
          self.options.afterError.call(
            self,
            "<!doctype html><html>" + doc.body.parentNode.innerHTML + "</html>"
          );
        }
        if (data) {
  ...
  function _createButton(arg) {
    ...
    var name = arg.name || "",
      span = K(
        '<span class="ke-button-common ke-button-outer" title="' +
          name +
          '"></span>'
      ),
      btn = K(
        '<input class="ke-button-common ke-button" type="button" value="' +
          name +
          '" />'
      );
    if (arg.click) {
  ...
  _extend(KDialog, KWidget, {
    init: function (options) {
      ...
      var contentDiv = K('<div class="ke-dialog-content"></div>').appendTo(
        self.div
      );
      if (_IE && _V < 7) {
        self.iframeMask = K(
      ...
      if (self.height) {
        bodyDiv.height(
          _removeUnit(self.height) - headerDiv.height() - footerDiv.height()
        );
      }
    ...
    create: function () {
      ...
      K.each(self.items, function (i, name) {
        if (name == "|") {
          ...
          htmlList.push(
            '<span class="ke-toolbar-icon ke-toolbar-icon-url ke-icon-' +
              name +
  ...
  function _create(expr, options) {
    ...
    options.themesPath = _undef(
      options.themesPath,
      options.basePath + "themes/"
    );
  ...
  _plugin("core", function (K) {
    ...
    self.beforeGetHtml(function (html) {
      if (_IE && _V <= 8) {
        ...
        html = html.replace(/(<input)((?:\s+[^>]*)?>)/gi, function (
          $0,
          $1,
          $2
        ) {
...
KindEditor.lang(
  {
    ...
    uploadSuccess: "上传成功。",
    cutError:
      "您的浏览器安全设置不允许使用剪切操作，请使用快捷键(Ctrl+X)来完成。",
    copyError:
      "您的浏览器安全设置不允许使用复制操作，请使用快捷键(Ctrl+C)来完成。",
    pasteError:
      "您的浏览器安全设置不允许使用粘贴操作，请使用快捷键(Ctrl+V)来完成。",
    ajaxLoading: "加载中，请稍候 ...",
...
KindEditor.plugin("autoheight", function (K) {
  ...
  function resetHeight() {
    ...
    self.resize(
      null,
      Math.max((K.IE ? body.scrollHeight : body.offsetHeight) + 76, minHeight)
    );
  }
  function init() {
...
KindEditor.plugin("filemanager", function (K) {
  var self = this,
    name = "filemanager",
    fileManagerJson = K.undef(
      self.fileManagerJson,
      self.basePath + "php/file_manager_json.php"
    ),
    imgPath = self.pluginsPath + name + "/images/",
    lang = self.lang(name + ".");
  function makeFileTitle(filename, filesize, datetime) {
    return (
      filename + " (" + Math.ceil(filesize / 1024) + "KB, " + datetime + ")"
    );
  }
    ...
    function bindEvent(el, result, data, createFunc) {
      var fileUrl = K.formatUrl(result.current_url + data.filename, "absolute"),
        dirPath = encodeURIComponent(
          result.current_dir_path + data.filename + "/"
        );
      if (data.is_dir) {
        el.click(function (e) {
    ...
    function createList(result) {
      ...
      for (var i = 0, len = fileList.length; i < len; i++) {
        ...
        var iconUrl = imgPath + (data.is_dir ? "folder-16.gif" : "file-16.gif"),
          ...
          cell0 = K(row[0].insertCell(0))
            .addClass("ke-cell ke-name")
            .append(img)
        ...
        K(row[0].insertCell(1))
          .addClass("ke-cell ke-size")
          .html(data.is_dir ? "-" : Math.ceil(data.filesize / 1024) + "KB");
        K(row[0].insertCell(2))
          .addClass("ke-cell ke-datetime")
          .html(data.datetime);
      }
    }
...
(function (K) {
  ...
  K.extend(KSWFUpload, {
      ...
      var settings = {
        ...
        upload_error_handler: function (file, errorCode, message) {
          if (file && file.filestatus == SWFUpload.FILE_STATUS.ERROR) {
            var itemDiv = K('div[data-id="' + file.id + '"]', self.bodyDiv).eq(
              0
            );
        ...
        upload_success_handler: function (file, serverData) {
          ...
          try {
            ...
            self.options.afterError.call(
              this,
              "<!doctype html><html>" + serverData + "</html>"
            );
          }
          if (data.error !== 0) {
            showError(
              itemDiv,
              K.DEBUG ? data.message : self.options.errorMessage
            );
            return;
          }
          file.url = data.url;
    ...
    appendFile: function (file) {
      var self = this;
      var itemDiv = K(
        '<div class="ke-inline-block ke-item" data-id="' + file.id + '"></div>'
      );
      ...
      K(
        ...
        .appendTo(statusDiv);
      K(
        '<div class="ke-message">' + self.options.pendingMessage + "</div>"
      ).appendTo(statusDiv);
  ...
  SWFUpload.prototype.initSettings = function () {
    this.ensureDefault = function (settingName, defaultValue) {
      this.settings[settingName] =
        this.settings[settingName] == undefined
          ? defaultValue
          : this.settings[settingName];
    };
    ...
    if (!this.settings.preserve_relative_urls) {
      this.settings.upload_url = SWFUpload.completeURL(
...
(function () {
  ...
  if (typeof SWFUpload === "function") {
    ...
    SWFUpload.queue.uploadCompleteHandler = function (file) {
      var user_upload_complete_handler = this.queueSettings
  ...
  self.plugin.table = {
    prop: function (isInsert) {
      ...
      if (table) {
        ...
        colsBox.val(
          table[0].rows.length > 0 ? table[0].rows[0].cells.length : 0
        );
        ...
        _setColor(
          colorBox.eq(1),
          K.toHex(table[0].style.backgroundColor || table[0].bgColor || "")
        );
        widthBox[0].focus();
        widthBox[0].select();
      }
